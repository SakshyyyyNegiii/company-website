...
export const AppointmentsDrawer: React.FC<AppointmentsDrawerProps> = ({ onBookNew }) => {
  ...
  const openWhatsAppWithAppointment = (appt: Appointment) => {
    const text = encodeURIComponent(
      `Hello Shrikant Jhanwar & Bitso Team! Regarding my transformation consultation appointment for ${appt.businessName} (Ref: ${appt.id?.slice(0, 6)}): ${appt.preferredDate} at ${appt.timeSlot}.`
    );
    window.open(
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between text-xs text-slate-400 pb-1">
                  <span>Showing {appointments.length} scheduled session{appointments.length > 1 ? 's' : ''}</span>
                  <a
                    href="#contact"
                    onClick={closeAppointmentsDrawer}
                ...
                {appointments.map((appt) => (
                  <div
                    key={appt.id}
                    className="p-5 sm:p-6 rounded-3xl bg-slate-950/80 border border-slate-800 hover:border-slate-700 transition-all space-y-4 shadow-md"
                  >
                    ...
                    <div className="grid grid-cols-2 gap-2.5 p-3.5 rounded-2xl bg-slate-900 border border-slate-800 text-xs">
                    ...
                    <div className="pt-2 flex items-center justify-between gap-3 text-xs">
                      ...
                      {appt.status !== 'cancelled' && (
                        <button
                          ...
                          className="text-slate-400 hover:text-rose-400 transition-colors font-medium"
                        >
                          Cancel Appointment
                        </button>
                      )}
